...
_*Σημείωση*_:Υπάρχει λάθος στον τύπο της εκφώνησης. Μέσα στο άθροισμα όλα τα u θα είναι τα u k. Αλλιώς θα προσθέτουμε i φορές το ίδιο πράγμα[[#references][[2]]].
...
$$ \Large p_{n+1} = p_n - \frac{F}{J} $$ όπου p, F πίνακες $$p = \begin{bmatrix} x & y \end{bmatrix}$$, $$F = \begin{bmatrix} f(x, y) \\ g(x, y) \end{bmatrix}$$ και J ο Ιακωβιανός πίνακας του F $$\Large J = \begin{bmatrix} \frac{\partial f(x,y)}{\partial x} & \frac{\partial f(x,y)}{\partial y} \\ \frac{\partial g(x,y)}{\partial x} & \frac{\partial g(x,y)}{\partial y} \end{bmatrix}$$
#+end_center
...
Επιπλέον για την μέθοδο θα χρειαστούμε μια αρχική προσέγγιση. Για να την βρούμε θα κάνουμε τα γραφήματα και θα βρόυμε τιμές κοντά εκει που τέμνονται οι παραστάσεις.

#+begin_center
#+attr_html: :width 900px
#+attr_odt: :width 900px
[[./ask7/graph.jpg]]
#+end_center
...
Η επίλυση του συγκεκριμένου προβήματος θα γίνεται στο αρχείο =run.m= καθώς υπάρχει αρχείο =newtonRaphson.m= που περιέχει την συνάρτηση που εφαρμόζει την μέθοδο Newton-Raphson.
...
Στην αρχή τις επανάληψης θα υπολογίσουμε τον αντίστροφο πίνακα του =df(x0)= και τον πίνακα =f(x0)=. Θα κάνουμε την πράξη και θα την βάλουμε στο res. Επίσης χρειάζεται να πάρουμε τον ανάστροφο του πίνακα που προκύπτει απο τον πολλαπλασιασμό του =injJ*F= για να έχει ίδιες διαστάσεις με το x0.
Τέλος θα ελένξουμε αν η διαφορά του res με το x0 είναι μικρότερη απο το tol για να σταματήσουμε την επανάληψη και θα θέσουμε x0 ίσο με το res.
Αν τελείωσει η επανάληψη και το i είναι ίσο με το n σημαίνει πως η μέθοδο απέτυχε να συγκλίνει οπότε θα το εκτυπώσουμε.
#+begin_src matlab
